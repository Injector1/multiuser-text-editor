...
class ConnectionManager:
    def __init__(self):
        ...
        self.available_files = \
            [f for f in listdir(self.base_dir)
             if isfile(join(self.base_dir, f))]
        self.active_connections: Dict[str, List[(WebSocket, str)]] =\
            dict.fromkeys(self.available_files, [])

    async def connect(
            self, websocket: WebSocket, client_id: str, file_name: str
    ):
        if file_name in self.available_files:
            response = {
                'content': open(f'{self.base_dir}/{file_name}').read(),
                'cursor': {'x': 0, 'y': 0}
            }
    ...
    def disconnect(self, websocket: WebSocket, client_id: str):
        for key in self.active_connections:
            if (websocket, client_id) in self.active_connections[key]:
                del self.active_connections[key][self.active_connections[key]
                    .index((websocket, client_id))]
    ...
    async def broadcast(self, file_name: str, data: str):
        ...
        response = {
            'content': '',
            'cursor': {'x': user_cursor['x'], 'y': user_cursor['y']}}

        for active_file in self.active_connections:
            if active_file == file_name:
                response['content'] = merge_file_content(
                    f'{self.base_dir}/{file_name}', serialized_json['value']
                )
